...
def point_cloud_cluster():
    ...
    centers = np.asarray(downpcd.points)
    ...
    colors = np.random.random(centers.shape)
    tree = KDTree(centers)
    dis, indices = tree.query(np.asarray(point_cloud.points))
    point_cloud.colors = o3d.utility.Vector3dVector(colors[indices])

    # np.save("data/redkitchen/centers.npy", np.asarray(point_cloud.points))
    # labels = point_cloud.cluster_dbscan(eps=0.01, min_points=10, print_progress=True)
    # labels = np.array(labels)
    # max_label = np.array(labels).max()
